...
// src/base-emoji.ts
var require_base_emoji = __commonJS({
  "src/base-emoji.ts"(exports, module) {
    var baseEmoji = {
      charset: [
        "\u{1F600}",
        "\u{1F603}",
      ...
      encode: (number) => {
        let result = "";
        do {
          result = baseEmoji.charset[number % baseEmoji.base] + result;
          number = Math.floor(number / baseEmoji.base);
        } while (number > 0);
        return result;
      },
      decode: (string) => {
        let result = 0;
        let spl = string.split("\\u");
        spl.forEach((e) => {
...
export default require_base_emoji();
